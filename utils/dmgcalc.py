...
def true_dmg_model2(model, basemin, basemax, items, item_vars, sp_vars, weapon, spellmod, mastery, crit=True):
    ...
    for i in range(6):
        ...
        dmgvars[i] = model.new_int_var(0, 2147483647, f"dmgvar")
        model.add(dmgvars[i] == dmg)
        damage[i] = model.new_int_var(0, 2147483647, f"damage_{elements[i]}")
        model.add_multiplication_equality(damage[i], [dmgvars[i],strdexvar])
...
def true_dmg_model_linsp(model, basemin, basemax, items, item_vars, sp_vars, weapon, spellmod, mastery):
    base = [0,0,0,0,0,0]
    baser = [0,0,0,0,0,0]
    for i in range(6):
        base[i] = (basemin[i] + basemax[i])/2
    ...
    for i in range(5):
        if base[i] != 0 or i<=1:
            skillpoint1 = model.new_int_var(0, 150, f"{sptypes[i]}1")
            model.add_max_equality(skillpoint1, [0, item_sp[i]])
            skillpoint2 = model.new_int_var(0, 150, f"{sptypes[i]}2")
            model.add_min_equality(skillpoint2, [150, skillpoint1])
            skillpoints[i+1] = skillpoint2

    strdexvar = model.new_int_var(187, 500, f"strdexvar")
    model.add(strdexvar == 187 + skillpoints[1] + skillpoints[2])
    ...
    for i in range(6):
        ...
        dmgvars[i] = dmg
    dmg = model.new_int_var(0, 10000000, f"dmgvar")
    model.add(dmg == sum(dmgvars))
    damage = model.new_int_var(0, 30000000000, f"damage")
    model.add_multiplication_equality(damage, [dmg,strdexvar,5]) # This will be 10x
